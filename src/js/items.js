...
function renderItemsGrid() {
  ...
  itemsToShow.forEach((item) => {
    ...
    if (typeof item.discountPrice === "number") {
      price.innerHTML = `PRICE : <span style=\"text-decoration:line-through; opacity:.7; color:#ffffffcc\">${Number(item.price).toFixed(2)}</span> <span>${Number(item.discountPrice).toFixed(2)}</span>`;
    } else {
